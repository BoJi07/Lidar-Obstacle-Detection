...
void helpCluster(std::vector<std::vector<float>> points, int id, KdTree* tree, float distanceTol, std::vector<bool> &processed, std::vector<int> &cluster){
  	 processed[id] = true;
  	 cluster.push_back(id);
 	 std::vector<int> nearPoint = tree->search(points[id],distanceTol);
  	 for(int &index : nearPoint){
       	if(!processed[index]){
			helpCluster(points,index,tree,distanceTol,processed,cluster);
        }
     }
}
 	...
 	std::vector<bool> processed(points.size(),false);
  	for(int i = 0;i<points.size(); i++){
     	if(!processed[i]){
         	...
         	helpCluster(points,i,tree,distanceTol,processed,cluster);
